...
{
  /* exploits ability of shifts to reduce x to the single byte n */
  int byte = (x >> (n << 3)) & 0xff;
  return byte;
}
...
int negate(int x)
{
  /* computes Two's Compliment of x */
  int output = ~x + 1;
  return output;
}
...
{
  /* exploits shift to reduce x to the sign bit and inverts it. Also checks for if x is 0 or 1. */ 
  int pos = !((x & (1 << 31)) | (!x));
  return pos;
}
...
int tmax(int n)
{
  /* shifts 1 over n - 1 bits and subtracts 1 which creates the sign bit 0 and the rest of the bits become 1 */ 
  int x = (1 << (n - 1)) - 1; 
  return x;
}
